...
import { requestMerchantQuoteTool } from "./tools/request-merchant-quote.tool.js";
import { compareMerchantOffersTool } from "./tools/compare-merchant-offers.tool.js";

export interface PurchaseAgentOptions {
    customerId?: string;
    conversationHistory?: Array<{ role: "user" | "assistant"; content: string }>;
}

export class PurchaseAgent {
    private tools = [requestMerchantQuoteTool, compareMerchantOffersTool];

    async execute(message: string, options: PurchaseAgentOptions = {}): Promise<string> {
        ...
        while (maxTurnCount > 0) {
            ...
            for (const item of functionCalls) {
                ...
                try {
                    switch (toolName) {
                        case "request_merchant_quote":
                            // Mocking merchant quote
                            toolResult = {
                                merchantId: args.merchantId,
                                availability: "all_available",
                                totalPrice: args.items.length * 100, // mock price
                                deliveryTimeMinutes: 30,
                            };
                            break;
                        case "compare_merchant_offers":
                            // Mocking comparison logic
                            toolResult = {
                                best_offer: args.quotes.length > 0 ? args.quotes[0].merchantId : null,
        ...
        return textOutput || "Purchase operation completed.";
    }
}

export const purchaseAgent = new PurchaseAgent();
export async function runPurchaseAgent(message: string, customerId?: string, history: any[] = []): Promise<string> {
    return purchaseAgent.execute(message, { customerId, conversationHistory: history });
}
