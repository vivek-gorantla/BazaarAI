...
import { deleteProduct, getCatalog, getCatalogProduct, getSearch, patchProduct, postBulkProducts, postProduct } from "../controllers/catalog.js";

const router = Router();
router.get("/search", asyncHandler(getSearch));
router.use(requireMerchant);
router.get("/:storeId", asyncHandler(getCatalog));
router.post("/:storeId/products/bulk", asyncHandler(postBulkProducts));
